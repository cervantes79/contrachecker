"""
ChainOfMeaning v2 OTOPSI RAPORU
Prolog-tarzi backward chaining motoru.
...
def run_scenario_c():
    ...
    # v2: inheritance resolves through parent types, so check for 'saglikli'
    # connected through 'elma' or 'meyve' (not just exact 'yesil elma' string)
    ...
    if conclusions:
        for c in conclusions:
            if c["type"] == "chained":
                has_any_chain = True
                chain = c["chain"]
                # Check if saglikli is reachable through elma or meyve
